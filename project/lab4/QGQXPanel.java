...
public class QGQXPanel extends JPanel {
  ...
  /*  private void drawMarks(Graphics g) {
    ...
    private void reSize() {
        ...
        this.yS = (int) (getHeight()*0.7);
        wB = getWidth() * (1F/2F)/ (xMax - xMin);
        hB = getHeight() *(3F/4F)/ (yMax - yMin);
    }

    private void drawView(int index, Graphics g, View view) {

        if (view.getResult() != null){
            float gen = view.getResult();

            Font font = g.getFont();
            g.setFont(new Font("",10,20));
            g.drawString("方程: x- 2sin(x) = 0  求得根--结果: "+gen,getWidth()-500,100);
            g.setFont(font);
            //画切线
            if (qx1 != null && qx2!= null){
                g.drawLine(castX(qx1.x),castY(qx1.y),castX(qx2.x),castY(qx2.y));
            }
        }
